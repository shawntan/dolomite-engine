...
def row_block_counts_and_sequence_ids(cu_seqlens: torch.Tensor, BLOCK_M: int, BLOCK_N: int):
    ...
    first_row_block = torch.where(
        M_block_start_ids == 0, 0,
        (cu_seqlens // BLOCK_N)[M_block_start_ids - 1]
    )
...
def compute_attn_weights(q, k, cm, neg_log_acc, qk_scale, mask, MASK: tl.constexpr):
    ...
    if MASK:
        ...
        p = tl.where(mask, p, 0.).to(p.dtype)
    else:
        _log_p += tl.dot(neg_log, cm, allow_tf32=ALLOW_TF32)
        p = tl.math.exp2(_log_p)
    return neg_log, p
...
def _forward(
        ...
        M_ptr, stride_mh, stride_mi, stride_mm, CRB_ptr,
    ...
    cm = tl.where(N_range[:, None] >= N_range[None, :], 1., 0.).to(Q_ptr.type.element_ty)
    ...
    for i in range(iters):
        ...
        neg_log, p, _, v = compute_block(
            q, neg_log_acc, min_start_idxs, start_idxs, token_size, cm, qk_scale,
            K_blk_ptrs, V_blk_ptrs, M_blk_idxs, N_blk_idxs,
            is_last_block, is_same_start, on_N_edge, on_band
        )
...
def compute_block(
        q, neg_log_acc, min_start_idxs, start_idxs, token_size, cm, qk_scale,
        K_blk_ptrs, V_blk_ptrs, M_blk_idxs, N_blk_idxs,
        is_last_block: tl.constexpr, is_same_start: tl.constexpr,
        on_N_edge: tl.constexpr, on_band: tl.constexpr):

    if on_N_edge:
        N_mask = N_blk_idxs < token_size
        k = tl.load(K_blk_ptrs, mask=N_mask[None, :], other=0.)
        v = tl.load(V_blk_ptrs, mask=N_mask[:, None], other=0.)
    else:
...
def _backward_dq(
        ...
        M_ptr, stride_mh, stride_mi, stride_mm, DM_ptr, CRB_ptr,
        batch_ptr, CSL_ptr,
        # L_ptr, stride_lm, stride_ln,
        logit_scale, batch_size, token_size,
        BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_D: tl.constexpr):
    ...
    do = tl.load(DO_blk_ptrs, mask=M_mask[:, None], other=0.)
    cm = tl.where(N_range[:, None] >= N_range[None, :], 1., 0.).to(Q_ptr.type.element_ty)
    dr = tl.load(DR_blk_ptrs, mask=M_mask, other=0.)
    ...
    for i in range(iters):
        ...
        neg_log, p, k, v = compute_block(q, neg_log_acc, min_start_idxs, start_idxs, token_size, cm, qk_scale, K_blk_ptrs, V_blk_ptrs,
                                         M_blk_idxs, N_blk_idxs, is_last_block, is_same_start, on_N_edge, on_band)
...
def _backward_dkdv(
        ...
        K_ptr, stride_kh, stride_kn, stride_kd,
        V_ptr, stride_vh, stride_vn, stride_vd,
        M_ptr, stride_mh, stride_mi, stride_mm, DM_ptr, CRB_ptr, FRB_ptr,
    ...
    cm = tl.where(N_range[:, None] >= N_range[None, :], 1., 0.).to(Q_ptr.type.element_ty)
    k = tl.load(K_blk_ptrs, mask=N_mask[None, :], other=0.)
    vT = tl.load(V_blk_ptrs, mask=N_mask[None, :], other=0.)
    ...
    for i in range(iters):
        ...
        on_M_edge = (M_block_id + 1) * BLOCK_M - 1 < token_size
        on_band = i == 0
        is_last_block = i == iters - 1
...
class StickBreakingAttention(torch.autograd.Function):
    ...
    def backward(ctx, do, drem):
        ...
        dq, dk, dv = sb_bwd(do, drem, q, k, v, cu_seqlens, M, sequence_ids, cu_row_blocks, first_row_block, logit_scale)
...
def sb_flash_attn_varlen(q, k, v, cu_seqlens, inv_temp=None, zero_start=True):
    ...
    with torch.no_grad():
        cu_row_blocks, first_row_block, sequence_ids = \
            row_block_counts_and_sequence_ids(cu_seqlens, BLOCK_M, BLOCK_N)
